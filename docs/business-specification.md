...
35. Record status is separate from snapshot lifecycle: `READY`, `NEEDS_REVIEW`, `PARTIAL`, and `EXCLUDED` describe records; `SETUP`, `PROCESSING`, `INCOMPLETE`, and `COMPLETE` describe the snapshot lifecycle. `SETUP` reserves a session identity before capture and is not an active capture/processing job.
